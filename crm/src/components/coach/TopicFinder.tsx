...
export default function TopicFinder({ onPick }: { onPick: (t: Topic) => void }) {
  ...
  const [selForm, setSelForm] = useState<string>("");
  ...
  async function handleGenerate(kw?: string) {
    ...
    if (!k) return;
    setKeyword(k);
    const res = await generate.mutateAsync({ keyword: k, perCategory, platforms: selPlatforms, form: selForm || undefined });
  ...
  return (
    <div className="rounded-xl border border-[var(--border,#e5e7eb)] bg-white p-4 mb-4">
      <h3 className="text-sm font-bold flex items-center gap-2 mb-3">
        💡 第一步 · 选题
        <span className="text-xs font-normal text-[var(--muted,#9ca3af)]">输入关键词，AI 帮你想高流量选题</span>
      </h3>
          ...
          {PLATFORM_OPTIONS.map((p) => {
            ...
            return (
              <button
                ...
                style={on ? { background: "#2563eb", color: "#fff", borderColor: "#2563eb" } : { background: "#fff", color: "#4b5563", borderColor: "var(--border,#e5e7eb)" }}
              >
                {p}
              </button>
            );
          ...
          {["", ...FORM_OPTIONS].map((f) => {
            ...
            return (
              <button
                ...
                className="rounded-full px-2.5 py-0.5 text-xs border transition"
                style={on ? { background: "#7c3aed", color: "#fff", borderColor: "#7c3aed" } : { background: "#fff", color: "#4b5563", borderColor: "var(--border,#e5e7eb)" }}
              >
                {f === "" ? "不限" : f === "视频" ? "🎬 视频" : "🖼 图文"}
              </button>
            );
      ...
      {generate.isError && (
        <p className="text-xs text-red-600 mt-2">{(generate.error as Error)?.message || "生成失败，请重试"}</p>
      )}

      {/* 历史批次 */}
      {batches.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 mt-3 pt-3 border-t border-[var(--border,#f0f0f0)]">
          <span className="text-xs text-[var(--muted,#9ca3af)]">历史：</span>
          {batches.slice(0, 10).map((b) => (
            <button
              ...
              className={`rounded-full px-2.5 py-0.5 text-xs border ${
                b.batchId === currentBatchId ? "bg-blue-600 text-white border-blue-600" : "border-[var(--border,#e5e7eb)] text-[var(--muted,#4b5563)] hover:border-blue-400"
              }`}
            >
              {b.keyword} · {b.count}
            </button>
          ...
          {categoriesInBatch.map((c) => {
            ...
            return (
              <button
                ...
                style={active ? { background: meta.color, color: "#fff", borderColor: meta.color } : { background: "#fff", color: meta.color, borderColor: "var(--border,#e5e7eb)" }}
              >
                {c === "全部" ? "全部" : `${meta.icon} ${c}`}
              </button>
            );
      ...
      {/* 选题卡片 */}
      {isLoading ? (
        <p className="text-sm text-[var(--muted,#9ca3af)] py-6 text-center">加载中…</p>
      ) : currentTopics.length === 0 ? (
        <div className="rounded-lg border border-dashed border-[var(--border,#e5e7eb)] py-8 text-center mt-3">
          <p className="text-2xl mb-1">💡</p>
          <p className="text-xs text-[var(--muted,#6b7280)]">输入关键词点「生成选题」，或下方直接手写主题生成内容</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 gap-2.5 mt-3">
          {currentTopics.map((t) => {
            ...
            return (
              <div
                key={t.id}
                className={`rounded-xl border p-3 flex flex-col gap-2 transition ${dropped ? "opacity-40" : "hover:shadow-sm"}`}
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  {t.status === "已生成" && <span className="text-[11px] text-green-600">✓ 已生成</span>}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      update.mutate({ id: t.id, content_form: form, ...(form === "图文" ? { needs_presenter: "不需要" } : {}) });
                    }}
                  ...
                  >
                    ...
                    {FORM_OPTIONS.map((f) => <option key={f} value={f}>{f === "视频" ? "🎬 视频" : "🖼 图文"}</option>)}
                  </select>
                  {t.content_form === "视频" && (
                    <select
                    ...
                    >
                      ...
                      {PRESENTER_OPTIONS.map((p) => (
                        <option key={p} value={p}>{p === "需要真人" ? "🧑 真人出镜" : p === "口播不出镜" ? "🎙 口播" : "🤖 无需出镜"}</option>
                      ))}
                    </select>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 mt-auto pt-1">
                  ...
                  <button
                    onClick={() => update.mutate({ id: t.id, status: dropped ? "待用" : "已弃用" })}
